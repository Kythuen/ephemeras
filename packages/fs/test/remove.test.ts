...
const TEMP_ROOT = join(homedir(), '.ephemeras/fs/Temp/remove')
  ...
  describe('## remove', () => {
    ...
    describe('### correct params', () => {
      ...
      it('#### remove file', async () => {
        ...
        const { all, done, skip } = await remove(TEST_FILE_TARGET)
      ...
      it('#### remove folder', async () => {
        ...
        const { all, done, skip } = await remove(TEST_FOLDER_TARGET)
      ...
      it('#### remove folder includes', async () => {
        ...
        expect(skip.length).toBe(targetItems.length - 3)
      ...
      it('#### remove folder includes excludes', async () => {
        ...
        expect(skip.length).toBe(targetItems.length - 2)
        expect(
          skip.every(i => !i.includes('sub2') || i.includes('1.txt'))
        ).toBeTruthy()
      })
      it('#### remove folder includes excludes filter', async () => {
        ...
        expect(done.every(i => i.includes('1.txt'))).toBeTruthy()
        expect(skip.length).toBe(targetItems.length - 1)
        expect(
          skip.every(i => !(i.includes('1.txt') && i.includes('sub1')))
        ).toBeTruthy()
      })
    })
  })
